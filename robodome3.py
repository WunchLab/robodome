...
## Hardcoded constants to be edited for each location

COM_PORT = 'COM39' #Windows device usb com  port, find in device manager
                   #device is called Prolific USB-to-Serial Comm Port

MET_FILE_RGX = 'C:/Users/Administrator/Desktop/em-27_aux_scripts/75_met//*.txt'    # Vaisala met files regex expression

LON, LAT = '-79.39', '43.66'  #Observer lon,lat

OUT_DIR = 'C:/Users/Administrator/Desktop/em-27_aux_scripts/robodome/robo_log/' #robodome log output directory

#--------------------------------------------------------------------------------------------------------
class Dome:#Class written by Jonathan Franklin, updated by L. Gillespie
           #changes included updating print statements, changed raw_input to input, and decoding
           #serial communication to utf-8.
    '''This is the virtual representation of the robodome,
    connecting the python program to the electronics.'''
    
    def __init__(self):
        self.tty=serial.Serial(COM_PORT, 9600, timeout=2) #CHANGE COM3 to serial port being used
    ...
    def readfrom(self):
        ...
        while loop:
            ...
            if n != 0:
                command = True
                text = text + self.tty.read(n).decode('utf-8')
                temp = text.split('\r')
...
def goodWeather(): #checks if weather meets good conditions
    ...
    out_dir = list_of_files = glob.glob(MET_FILE_RGX) #### set @ top of file, met file regular expression
...
#prevent dome from missing messages
	...
	while positionAccurate(str(position))==False and count <=3:
		move(((57.2958*position)-180)*0.85 +180)
		sleep(30)
		dome.write_command('GINF')
		result,message = dome.readfrom()
		if result == 0:
		...
		count +=1
		if count == 3:
			print('Dome didnt listen! Going to home....')
			dome.write_command('GHOM') #go to home if it's not listening
			sleep(120) #could be lessened....
			dome.write_command('GOPN') 
			sleep(30)

#------------------------------------------------------------------------

def automate(): #main function for automation#initialize
    #initialize ephemeris
    #pos = ephem.city("Toronto")#need to enter exact coordinates
    pos=ephem.Observer()
     
    pos.lon, pos.lat= LON, LAT  #Critical constants, should be at top of file for easy editing!!!
    ...
    try:
        ...
        if dome.shutter == 'Unknown': #send dome to starting position if shutter is unknown
            print("Dome shutter status unknown, going home and closing!")
        ...
        if goodWeather() == True and dome.shutter == 'Closed' and sun.alt>0.15: #send Dome to sun position if weather permits and daytime
            ...
            move(((57.2958*float(sun.az))-180)*0.85 + 180) #scaling to cover a smaller range to avoid shadows in morning and evenings 
            sleep(30)
        ...
        while True: #looping program
            ...
            if dome.shutter == 'Opened':
                if sun.alt <0.05: #closing for night time
                    dome.write_command('GHOM')
                    sleep(30)
                    dome.write_command('GCLS')
                    for i in range(60):
                ...
                if goodWeather() == True  and sun.alt>0.15: #opens dome again if conditions allow it
                    ...
                    for i in range(30):
                        sleep(1)
                    move(((57.2958*float(sun.az))-180)*0.85 + 180)
                    sleep(30)
...
out_dir = OUT_DIR #output directory for the log file
...
if __name__ == '__main__': #written by Jonathan Franklin, rewritten by L.Gillespie
...
#-------------------------------------------
#hope and pray it was that easy...
